define(
		...
		function(_, $) {
			/*******************************************************************
			 * 音符漂浮插件 ----------------------------- 作者：叼怎么写！- -|| 时间：2014-03-21
			 * 准则：zepto 联系：wechat--shoe11414255 一张网页，要经历怎样的过程，才能抵达用户面前
			 * 一个特效，要经历这样的修改，才能让用户点个赞 一个产品，创意源于生活，源于内心，需要慢慢品味
			 * ********************************************************************************************
			 * 这是别人写的东西，我只是重复利用，微调了下--努力努力 ^-^||
			 * 
			 * -----------保持队形------------------ <div id='coffee'></div>
			 ******************************************************************/
			// Zepto.js
			...
			(function($, undefined) {
				var prefix = '', eventPrefix, endEventName, endAnimationName, vendors = {
					...
					Moz : '',
					O : 'o'
				}, document = window.document, testEl = document
						.createElement('div'), supportedTransforms = /^((translate|rotate|scale)(X|Y|Z|3d)?|matrix(3d)?|perspective|skew(X|Y)?)$/i, transform, transitionProperty, transitionDuration, transitionTiming, transitionDelay, animationName, animationDuration, animationTiming, animationDelay, cssReset = {}
				...
				$.fn.animate = function(properties, duration, ease, callback,
					...
					if ($.isFunction(duration))
						callback = duration, ease = undefined,
								duration = undefined
					if ($.isFunction(ease))
				...
				$.fn.anim = function(properties, duration, ease, callback,
					...
					if (typeof properties == 'string') {
						...
						if (duration > 0 && typeof properties === 'object') {
							cssValues[transitionProperty] = cssProperties
									.join(', ')
					...
					wrappedCallback = function(event) {
						if (typeof event !== 'undefined') {
							if (event.target !== event.currentTarget)
								return // makes sure the event didn't bubble
								// from "below"
								$(event.target).unbind(endEvent,
										wrappedCallback)
						} else
							$(this).unbind(endEvent, wrappedCallback) // triggered
							// by
							// setTimeout
					...
					if (duration > 0) {
						...
						// transitionEnd is not always firing on older Android
						// phones
			...
			(function($) {
				...
				$.fn.coffee = function(option) {
					...
					var $coffeeSteamBox = $(
							'<div class="coffee-steam-box"></div>').css({
					...
					function steam() {
						...
						// 生成fly飞行体
						var $fly = $('<span class="coffee-steam">'
								+ randoms(1, opts.steams) + '</span>');
						var left = rand(0, coffeeSteamBoxWidth
								- opts.steamWidth - fontSize);
						if (left > position)
						...
						$fly
								...
								.animate(
										...
										rand(opts.steamFlyTime / 2,
												opts.steamFlyTime * 1.2),
										__flyFastSlow, function() {
											$fly.remove();
											$fly = null;
										});
					}
				...
				$.fn.coffee.defaults = {
					steams : [ 'jQuery', 'HTML5', 'HTML6', 'CSS2', 'CSS3',
							'JS', '$.fn()', 'char', 'short', 'if', 'float',
							'else', 'type', 'case', 'function', 'travel',
							'return', 'array()', 'empty()', 'eval', 'C++',
							'JAVA', 'PHP', 'JSP', '.NET', 'while', 'this',
							'$.find();', 'float', '$.ajax()', 'addClass',
							'width', 'height', 'Click', 'each', 'animate',
							'cookie', 'bug', 'Design', 'Julying', '$(this)',
							'i++', 'Chrome', 'Firefox', 'Firebug', 'IE6',
							'Guitar', 'Music', '攻城师', '旅行', '王子墨', '啤酒' ], /* 漂浮物的类型，种类 */
					steamsFontFamily : [ 'Verdana', 'Geneva', 'Comic Sans MS',
							'MS Serif', 'Lucida Sans Unicode',
							'Times New Roman', 'Trebuchet MS', 'Arial',
			...
			var WAudioCoffee = {
				init : function($el) {
					...
					this.audioNode.on('tap', _
							.bind(this.onBtnFlipClicked, this));
					...
					$('#coffee_flow')
							.coffee(
									{
										steams : [
												"<img src='theme/common/audio_0/audio_widget_01@2x.png' />",
												"<img src='theme/common/audio_0/audio_widget_01@2x.png' />" ],
										steamHeight : 100,
										steamWidth : 44,
										'z-index' : 100,
